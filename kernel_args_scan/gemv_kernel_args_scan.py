...
def test_gemm(m, n, k, warmup_times, benchmark_times, blockDimX, KernelId, KernelPara1, KernelParam2, exec_times):
    ...
    for _ in range(warmup_times):
        for _ in range(exec_times):
            C = muxi_layout_kernels.gemv_layoutA_wapper(A_transposed, B, m, k, alpha, beta, blockDimX, KernelId, KernelParam1, KernelParam2)
    torch.cuda.synchronize()
...
# Save results to CSV
csv_file = "MUXI_GEMV_MFU" + "_scan_result.csv"
...
csv_file = "MUXI_GEMV_MFU" + "_scan_best_args.csv"
with open(csv_file, mode='w', newline='') as file:
    writer = csv.writer(file)
