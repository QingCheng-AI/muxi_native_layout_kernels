...
def test_gemm(m, n, k, warpup_times, benchmark_times, exec_times, kernelParam1, kernelParam2, kernelParam3, kernelId):
    # Initialize tensors
    A = torch.randn(m, k, device='cuda', dtype=torch.bfloat16) * 2 - 1
    B = torch.randn(n, k, device='cuda', dtype=torch.bfloat16) * 2 - 1
    ...
    A_scale = torch.ones(m // 128, k // 128, dtype=torch.float32, device='cuda')

    # Warm-up for gemm_layoutABC
    for _ in range(warpup_times):
        for _ in range(exec_times):
            C = muxi_layout_kernels.gemm_layoutA_soft_fp8_wapper(A_transposed, A_scale, B, m, n, k, alpha, beta, kernelParam1, kernelParam2, kernelParam3, kernelId)
        torch.cuda.synchronize()
...
# m = [7168]
# k = [256]
m = [3072, 7168, 4608, 7168, 7168, 512, 2112]
k = [1536, 2048, 7168, 2304, 256, 7168, 7168]
# m = [512, 7168]
# k = [7168, 256]
max_n = 128
...
# Save results to CSV
csv_file = "MUXI_GEMM_MFU" + "_scan_result_all_kernel_layoutA_soft_fp8.csv"
...
csv_file = "MUXI_GEMM_MFU" + "_scan_best_args_all_kernel_layoutA_soft_fp8.csv"
with open(csv_file, mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(["m", "n", "k", "kernelId", "kernelParam1", "kernelParam2", "kernelParam3"])
    writer.writerows(best_result)

csv_file = "MUXI_GEMM_MFU" + "_scan_best_data_all_kernel_layoutA_soft_fp8.csv"
with open(csv_file, mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(["m", "n", "k", "average_gemmex_time","average_layoutABC_time", "mfu_layout", "kernelParam1", "kernelParam2", "kernelParam2", "kernelId", "max_diff_value"])
