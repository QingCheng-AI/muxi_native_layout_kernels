...
#include "muxi_hgemm_layoutC.cuh"
#include "utils.cuh"

namespace muxi_layout_kernels {

torch::Tensor muxi_hgemm_layoutC(torch::Tensor A, torch::Tensor B, float alpha,
                                 float beta,
    ...
    if (A.dtype() == torch::kFloat16) {
        if (isBetaZero) {
            if (hasOneDimBias) {
                layout_hgemm_tn_128x128x128_4m1n8k_256t_layoutC<
                    __half, __half, float, true, true><<<gird, 256>>>(
                ...
                layout_hgemm_tn_128x128x128_4m1n8k_256t_layoutC<
                    __half, __half, float, true, false><<<gird, 256>>>(
                    reinterpret_cast<__half *>(A.data_ptr<at::Half>()),
                    reinterpret_cast<__half *>(B.data_ptr<at::Half>()),
                    reinterpret_cast<__half *>(C.data_ptr<at::Half>()), m, n, k,
                    lda, ldb, ldc, alpha, beta);
            ...
            if (hasOneDimBias) {
                layout_hgemm_tn_128x128x128_4m1n8k_256t_layoutC<
                    __half, __half, float, false, true><<<gird, 256>>>(
                    reinterpret_cast<__half *>(A.data_ptr<at::Half>()),
                    reinterpret_cast<__half *>(B.data_ptr<at::Half>()),
                    reinterpret_cast<__half *>(C.data_ptr<at::Half>()), m, n, k,
                    lda, ldb, ldc, alpha, beta,
                ...
                layout_hgemm_tn_128x128x128_4m1n8k_256t_layoutC<
                    __half, __half, float, false, false><<<gird, 256>>>(
                    reinterpret_cast<__half *>(A.data_ptr<at::Half>()),
                    reinterpret_cast<__half *>(B.data_ptr<at::Half>()),
                    reinterpret_cast<__half *>(C.data_ptr<at::Half>()), m, n, k,
                    lda, ldb, ldc, alpha, beta);
        ...
        if (isBetaZero) {
            if (hasOneDimBias) {
                layout_hgemm_tn_128x128x128_4m1n8k_256t_layoutC<
                    ...
                    <<<gird, 256>>>(reinterpret_cast<__maca_bfloat16 *>(
