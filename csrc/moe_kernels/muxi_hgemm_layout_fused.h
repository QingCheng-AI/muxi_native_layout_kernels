...
layout_hgemm_tn_128x128x128_4m1n8k_256t_device_fused(
    const void *A, const void *B, void *C, int M, int N, int K, int lda,
    int ldb, int ldc, Tc *score_weights, Tscal beta, const void *bias, int bidx,
    int bidy) {
    ...
    for (int j = 0; j < 4; j++) {
        for (int i = 0; i < 4; i++) {
            ...
            if ((startCol + (slot & 1) * 64 + j * 16) < N) {
                float C_f32_res[4];
                int weight_index =
                    C_offset * (sizeof(CStgType) / sizeof(Tc)) / M;
                Tscal alpha = fp_cast<Tscal>(score_weights[weight_index]);
#pragma unroll 4
                for (int t = 0; t < 4; t++) {
                ...
                if constexpr (!IsBetaZero) {
                    ...
                    C_f32_res[0] += beta * fp_cast<Tscal>(C_tmp_ptr[0]);
                    C_f32_res[1] += beta * fp_cast<Tscal>(C_tmp_ptr[1]);
                    C_f32_res[2] += beta * fp_cast<Tscal>(C_tmp_ptr[2]);
                    C_f32_res[3] += beta * fp_cast<Tscal>(C_tmp_ptr[3]);
                }
                Tc C_tc_tmp[4];
#pragma unroll 4
                for (int t = 0; t < 4; t++) {
...
template <typename T, typename Tc, typename Tscal, bool IsBetaZero,
          bool HasOneDimBias>
__global__ void layout_hgemm_tn_128x128x128_4m1n8k_256t_fused(
    const void *A, const void *B, void *C, int M, int N, int K, int lda,
    int ldb, int ldc, Tc *score_weights, Tscal beta,
    const void *bias = nullptr) {
    layout_hgemm_tn_128x128x128_4m1n8k_256t_device_fused<
        T, Tc, Tscal, IsBetaZero, HasOneDimBias>(A, B, C, M, N, K, lda, ldb,
                                                 ldc, score_weights, beta, bias,
