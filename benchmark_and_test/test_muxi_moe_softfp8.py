...
def test_fused_moe(
    ...
    a1 = a.clone()
    w1 = (torch.rand((e, 2 * n, k), device="cuda", dtype=dtype) / 10).to(
        torch.float8_e4m3fn
    )
    w2 = (torch.rand((e, k, n), device="cuda", dtype=dtype) / 10).to(
        torch.float8_e4m3fn
    )

    w1_scale = (
        torch.rand((e, 2 * n // 128, k // 128), device="cuda", dtype=torch.float32) * 2
    )
    w2_scale = (
        torch.rand((e, k // 128, n // 128), device="cuda", dtype=torch.float32) * 2
    )
