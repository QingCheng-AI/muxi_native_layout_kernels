...
def test_gemm(m, n, k, warpup_times, benchmark_times, exec_times):
    ...
    bias = torch.randn(1, m, device='cuda', dtype=torch.bfloat16)
    ...
    for _ in range(warpup_times):
        for _ in range(exec_times):
            ...
            C = muxi_layout_kernels.muxi_hgemm_layoutA(A_transposed, B, alpha, beta, scale_matrix, bias = bias)
    torch.cuda.synchronize()
...
m = [3072, 7168, 7168, 512, 4608, 7168]
k = [1536, 2048, 256, 7168, 7168, 2304]
