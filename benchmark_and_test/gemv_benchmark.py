...
def test_gemm(m, n, k, warpup_times, benchmark_times, exec_times):
    ...
    B = torch.randn(n, k, device='cuda', dtype=dtype) / 10
    C = torch.zeros(n, m, device='cuda', dtype=dtype)
    # bias = torch.rand(1, m, device='cuda', dtype=dtype)
    bias = torch.randn(1, m, device='cuda', dtype=dtype)
    ...
    for _ in range(warpup_times):
        for _ in range(exec_times):
            C = muxi_layout_kernels.gemv_layoutA(A_transposed, B, alpha, beta, bias=bias)
            # C = muxi_layout_kernels.gemv_layoutA_wapper(A_transposed, B, m, k, alpha, beta, 256, 1, 1, 1, bias=bias)
    torch.cuda.synchronize()
...
#Qwen2_7B shapes
m = [37888, 4608, 3584, 3584]
k = [3584, 3584, 3584, 18944]

# #deepseek R1 671B shapes
# m = [2112, 3072, 7168, 7168]
# k = [7168, 1536, 2048, 256]
