...
def test_gemm(m, n, k, warpup_times, benchmark_times, exec_times):
    # Initialize tensors
    A = torch.ones(m, k, device='cuda', dtype=torch.float16) * 2 - 1
    B = torch.ones(n, k, device='cuda', dtype=torch.float16) * 2 - 1
    bias = torch.rand(1, m, device='cuda', dtype=torch.float16)
    ...
    for _ in range(warpup_times):
        for _ in range(exec_times):
            ...
            C = muxi_layout_kernels.gemm_layoutA(A_transposed, B, alpha, beta, bias)
            # C = muxi_layout_kernels.gemm_layoutAB_ContinuousC(A_transposed, B_transposed, alpha, beta, bias)
            # C = muxi_layout_kernels.muxi_hgemm_layoutC(A_transposed, B_transposed, alpha, beta, bias)
    torch.cuda.synchronize()
