...
@pytest.mark.parametrize("m", [8, 16, 32, 48, 64, 80, 96, 112, 128, 144, 160, 176, 192, 208, 224, 240, 256, 272, 384, 512, 1024]) # batchSize
...
def test_fused_moe(
    ...
    maca_output = planC_moe(a, w1_transposed, w2_transposed, score, topk, renormalize=True, use_grouped_topk=True, num_expert_group=8, topk_group=4, gating_bias=bias, score_func=score_func)

    iterative_output = iterative_moe(a1, w1, w2, score, topk, 8, 4, global_num_experts=e, renormalize=True, e_score_correction_bias=bias, score_func=score_func)    


    # print(maca_output.shape)
    # print(iterative_output.shape)
    # print(maca_output)
